-- Fix: assignments (Opfer-Zuweisung) were missing from migrations 007 + 010.
-- This restores the full start_game with all features:
--   - admin-owned task pool + disabled list
--   - difficulty ordering (1+2 before 3)
--   - assignment ring (every player gets a target)
...
BEGIN
  ...
  -- Assign tasks (difficulty-ordered, then random within each group)
  ...
  -- Create assignment ring: player 1 → 2 → 3 → ... → N → 1
  FOR i IN 1..v_n_players LOOP
